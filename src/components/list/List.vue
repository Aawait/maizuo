<template>
  ...
  <div class="wrapper" ref="wrapper" v-else>
    <div class="content">
      <ul class="list" ref="showlist">
        <li
          v-for="(item, index) in data"
          class="listitem"
          :key="index"
          @click="goDetails(item.filmId)"
        >
          <div class="item-left">
          ...
          <div class="item-middle">
            ...
            <div class="middle-coun" v-if="option.params.type == 2">
              <span>上映时间：</span
              ><span>{{ item.premiereAt | formatTime }}</span>
            </div>
          </div>
          ...
          <div
            class="item-right"
            v-if="option.params.type == 2 && item.isPresale"
          >
...
export default {
  ...
  mounted() {
         ...
         // 因为有v-if 异步请求任务完成 列表才会显示，所以用定时器不然获取不到DOM
         setTimeout(()=>{

             this.scroll = new BScroll(this.$refs.wrapper,{
               click: true,  // 允许scroll盒子内点击事件
               observeDOM: true,  // 
               pullUpLoad: true  // 开启下拉触底事件
            })
           ...
           this.scroll.on('pullingUp',()=>{
              ...
              if(this.flag){
                 ...
                 setTimeout(()=>{
                     ...
                     this.scroll.finishPullUp()  // 告诉scroll操作完了，下次还能继续触发该事件
                 },2000)
              }
          })
         },1500)
